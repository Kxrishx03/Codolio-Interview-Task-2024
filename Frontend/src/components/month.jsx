...
export function Month({ sample }) {
    ...
    function filterByCategory() {
        const filtered = filteredMonth.map(month =>
            month.map(day =>
                day.filter(transaction => transaction.category.toLowerCase() === filtervalues.category.toLowerCase())
            )
        );
        setFilteredMonth(filtered);
        console.log(filtered);
    ...
    function filterByCurrency() {
        const filtered = filteredMonth.map(month =>
            month.map(day =>
                day.filter(transaction => transaction.currency.toLowerCase() === filtervalues.currency.toLowerCase())
            )
        );
        setFilteredMonth(filtered);
        console.log(filtered);
    ...
    return (
        <>
            ...
            <div className={`flex flex-col justify-center items-center w-full ${lightTheme ? 'bg-neutral-200 text-black' : 'bg-gray-800 text-white'}`}>
